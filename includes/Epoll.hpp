...
class Epoll
{
	public:
		...
		typedef	std::map<t_socket, int>	stockClientType;
		...
		void	addClient(t_socket const & sock, int const & infoServ);
		...
		std::map<t_socket, int>				&getSockClient();
	...
	private:
		...
		std::map<t_socket, int>		_sockClient;
